...
# #### Preprocessing ----
outdir <-
  file.path(outdir, selected_cell_type, str_c("k_", k), "FTX_deg")
...
table(metacell_obj@assays$RNA@counts['FTX', ] > median(metacell_obj@assays$RNA@counts['FTX', ]))
...
enriched <- deg_metacell_obj |>
  filter(group == "FTX") |>
  cat_enrich()   
saveRDS(enriched, file.path(outdir, "FTX_enriched.rds"))
