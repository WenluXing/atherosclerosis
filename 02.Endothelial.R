...
output.dir <- paste0("./results-V24-Figure/Fig2_End") 
...
ggsave(file.path(output.dir, "Fig2B_san_MALAT1_chemotaxis.pdf"),
       plot = P2,
       height = 2.5,
       width = 2.5)
#
...
P2 <- ggMarginal(P, type = "density", 
                 xparams = list(fill = "orange"),
                 yparams = list(fill = "#00AFBB"))
ggsave(file.path(output.dir, "Fig2B_san_MALAT1_chemotaxis2.pdf"),
       plot = P2,
       height = 2.5,
       width = 2.5)
#
library(ggExtra)
P <- expr %>%
  ggplot(aes(x = MALAT1, y = GOBP_ENDOTHELIAL_CELL_PROLIFERATION)) +
  geom_point(size = 0.5,color="black",alpha = 0.9) + 
  geom_smooth(method = "lm", formula = y~x, color = "#756bb1", fill = "#cbc9e2")+
  ggpubr::stat_cor(method = 'pearson',
                   aes(x = MALAT1, y = GOBP_ENDOTHELIAL_CELL_PROLIFERATION),
                   label.x.npc = "left",
                   label.y.npc = "top",
                   size = 3,
                   color = "black") +
  ...
  theme(aspect.ratio = 1, legend.margin = margin(l=-8),
        axis.title.x = element_text(size = 8,face = "italic", color = "black"),
        axis.title.y = element_text(size = 8, color = "black")) +
  labs(y = "GOBP_ENDOTHELIAL_CELL_PROLIFERATION" |>
         str_replace_all("GOBP_", "") |>
         str_replace_all("_", " ") |>
         str_to_sentence())+
  NoLegend()
P2 <- ggMarginal(P, type = "density", 
                 xparams = list(fill = "orange"),
                 yparams = list(fill = "#00AFBB"))
ggsave(file.path(output.dir, "FigS2C_san_MALAT1_proliferation.pdf"),
       plot = P2,
       height = 2.5,
       width = 2.5)
#
library(ggExtra)
P <- expr %>%
  ggplot(aes(x = MALAT1, y = GOBP_ENDOTHELIAL_CELL_MIGRATION)) +
  geom_point(size = 0.5,color="black",alpha = 0.9) + 
  geom_smooth(method = "lm", formula = y~x, color = "#756bb1", fill = "#cbc9e2")+
  ggpubr::stat_cor(method = 'pearson',
                   aes(x = MALAT1, y = GOBP_ENDOTHELIAL_CELL_MIGRATION),
                   label.x.npc = "left",
                   label.y.npc = "top",
                   size = 3,
                   color = "black") +
  ...
  theme(aspect.ratio = 1, legend.margin = margin(l=-8),
        axis.title.x = element_text(size = 8,face = "italic", color = "black"),
        axis.title.y = element_text(size = 8, color = "black")) +
  labs(y = "GOBP_ENDOTHELIAL_CELL_MIGRATION" |>
         str_replace_all("GOBP_", "") |>
         str_replace_all("_", " ") |>
         str_to_sentence())+
  NoLegend()
P2 <- ggMarginal(P, type = "density", 
                 xparams = list(fill = "orange"),
                 yparams = list(fill = "#00AFBB"))
ggsave(file.path(output.dir, "FigS2F_san_MALAT1_migration.pdf"),
       plot = P2,
       height = 2.5,
       width = 2.5)
#
...
P2 <- ggMarginal(P, type = "density", 
                 xparams = list(fill = "orange"),
                 yparams = list(fill = "#00AFBB"))
ggsave(file.path(output.dir, "Fig2B_san_PCAT19_adhension.pdf"),
       plot = P2,
       height = 2.5,
       width = 2.5)
#
...
P2 <- ggMarginal(P, type = "density", 
                 xparams = list(fill = "orange"),
                 yparams = list(fill = "#00AFBB"))
ggsave(file.path(output.dir, "Fig2B_san_PCAT19_adhension2.pdf"),
       plot = P2,
       height = 2.5,
       width = 2.5)
#
...
P2 <- ggMarginal(P, type = "density", 
                 xparams = list(fill = "orange"),
                 yparams = list(fill = "#00AFBB"))
ggsave(file.path(output.dir, "Fig3A_san_MALAT1_vasculogenesis.pdf"),
       plot = P2,
       height = 2.5,
       width = 2.5)

## Fig2D FigS2D FigS2G FigS3B ----
adata <-
...
p <- ggdotchart(a, x = "feature_y", y = "estimate",
  ...
  scale_color_manual(values = c(low = "#56B4E9",
                                high = "#FE8D3C"))
ggsave(file.path(output.dir, "Fig2D_bang_MALAT1_chemotaxis.pdf"),
       plot = p,
       height = 2.5,
       width = 4)
...
p <- ggdotchart(a, x = "feature_y", y = "estimate",
  ...
  scale_color_manual(values = c(low = "#00AFBB",
                                high = "orange"))
ggsave(file.path(output.dir, "Fig2D_bang_PCAT19_adhesion.pdf"),
       plot = p,
       height = 2.5,
       width = 4)
...
#
selected_features <- gene_sets %>%
  filter(gs_name == "GOBP_ENDOTHELIAL_CELL_PROLIFERATION") %>%
  pull(gene_symbol)
a <- filtered_adata %>%
  filter(feature_y %in% selected_features,
         feature_x == "MALAT1") %>%
...
p <- ggdotchart(a, x = "feature_y", y = "estimate",
                ...
                xlab="",
                dot.size = 2.5)+                              
  labs(title = "GOBP_ENDOTHELIAL_CELL_PROLIFERATION"|>
         str_replace_all("GOBP_", "") |>
         str_replace_all("_", " ") |>
         str_to_sentence(),
       y = "Correlation of MALAT1") +
  ...
  scale_color_manual(values = c(low = "#56B4E9",
                                high = "#FE8D3C"))
ggsave(file.path(output.dir, "FigS2D_bang_MALAT1_proliferation.pdf"),
       plot = p,
       height = 2.5,
       width = 4)
#
selected_features <- gene_sets %>%
  filter(gs_name == "GOBP_ENDOTHELIAL_CELL_MIGRATION") %>%
  pull(gene_symbol)
a <- filtered_adata %>%
  filter(feature_y %in% selected_features,
         feature_x == "MALAT1") %>%
...
p <- ggdotchart(a, x = "feature_y", y = "estimate",
                ...
                xlab="",
                dot.size = 2.5)+                              
  labs(title = "GOBP_ENDOTHELIAL_CELL_MIGRATION"|>
         str_replace_all("GOBP_", "") |>
         str_replace_all("_", " ") |>
         str_to_sentence(),
       y = "Correlation of MALAT1") +
  ...
  scale_color_manual(values = c(low = "#56B4E9",
                                high = "#FE8D3C"))
ggsave(file.path(output.dir, "FigS2G_bang_MALAT1_migration.pdf"),
       plot = p,
       height = 2.5,
       width = 4)
...
p <- ggdotchart(a, x = "feature_y", y = "estimate",
  ...
  scale_color_manual(values = c(low = "#00AFBB",
                                high = "orange"))
p  
ggsave(file.path(output.dir, "FigS3B_bang_MALAT1_vasculogensis.pdf"),
       plot = p,
       height = 2.5,
       width = 4)

## FigS2A FigS2B FigS3C FigS3D ----
selected_genes <- c("CYTOR","CD99","GAS6","ANXA1","CEACAM1","VEGFC","HES1",
                    "HEY1","RAMP2","CTNNB1","RRAS","EMP2","CAV1","TMEM100","HDAC7","HEY2","TIE1","NTRK2","CEACAM1","RAPGEF2","QKI",
                    "RGCC","GATA2","TIE1",  "FLT1","PGF",#
                    "LEF1","HMGB1",'CX3CL1',"HLA-DRB5","PCAT19",
...
ggsave(file.path(output.dir, "FigS2A_san_MALAT1_CXCL12.pdf"),
       plot = P2,
       height = 2.5,
       width = 2.5)
#
P <- expr %>%
  ggplot(aes(x = MALAT1, y = CX3CL1)) +
  geom_point(size = 0.5,color="black",alpha = 0.9) + 
  geom_smooth(method = "lm", formula = y~x, color = "#756bb1", fill = "#cbc9e2")+
  ggpubr::stat_cor(method = 'pearson',
                   aes(x = MALAT1, y = CX3CL1),
                   label.x.npc = "left",
                   label.y.npc = "top",
                   size = 3,
                   color = "black") +
...
P2 <- ggMarginal(P, type = "density", 
                 xparams = list(fill = "pink"),
                 yparams = list(fill = "#FAE6BE"))
ggsave(file.path(output.dir, "FigS2A_san_MALAT1_CX3CL1.pdf"),
       plot = P2,
       height = 2.5,
       width = 2.5)
#
P <- expr %>%
  ggplot(aes(x = MALAT1, y = HMGB1)) +
  geom_point(size = 0.5,color="black",alpha = 0.9) + 
  geom_smooth(method = "lm", formula = y~x, color = "#756bb1", fill = "#cbc9e2")+
  ggpubr::stat_cor(method = 'pearson',
                   aes(x = MALAT1, y = HMGB1),
                   label.x.npc = "left",
                   label.y.npc = "top",
                   size = 3,
                   color = "black") +
...
P2 <- ggMarginal(P, type = "density", 
                 xparams = list(fill = "pink"),
                 yparams = list(fill = "#FAE6BE"))
ggsave(file.path(output.dir, "FigS2A_san_MALAT1_HMGB1.pdf"),
       plot = P2,
       height = 2.5,
       width = 2.5)
...
P2 <- ggMarginal(P, type = "density", 
                 xparams = list(fill = "pink"),
                 yparams = list(fill = "#FAE6BE"))
ggsave(file.path(output.dir, "FigS2B_san_PCAT19_CD74.pdf"),
       plot = P2,
       height = 2.5,
       width = 2.5)
...
P2 <- ggMarginal(P, type = "density", 
                 xparams = list(fill = "pink"),
                 yparams = list(fill = "#FAE6BE"))
ggsave(file.path(output.dir, "FigS2B_san_PCAT19_ETS1.pdf"),
       plot = P2,
       height = 2.5,
       width = 2.5)
...
P2 <- ggMarginal(P, type = "density", 
                 xparams = list(fill = "pink"),
                 yparams = list(fill = "#FAE6BE"))
ggsave(file.path(output.dir, "FigS2B_san_PCAT19_HES1.pdf"),
       plot = P2,
       height = 2.5,
       width = 2.5)
...
#                  xparams = list(fill = "pink"),
#                  yparams = list(fill = "#FAE6BE"))
ggsave(file.path(output.dir, "FigS3C_san_MALAT1_QKI.pdf"),
       plot = P,
       height = 2.5,
       width = 2.5)
#
...
#                  xparams = list(fill = "pink"),
#                  yparams = list(fill = "#FAE6BE"))
ggsave(file.path(output.dir, "FigS3D_san_MALAT1_HDAC7.pdf"),
       plot = P,
       height = 2.5,
       width = 2.5)

## Fig2E FigS2E FigS2H FigS3E ----
adata <-
  readRDS("./results-V4-correlation/Endothelial/k_20/correlation/GO:BP.scores.rds")
seurat_obj <-
...
boxplot <- ggpaired(a, 
  ...
  NoLegend() 
boxplot
ggsave(file.path(output.dir, "Fig2E_deg_MALAT1_chemotaxis.pdf"),
       plot = boxplot,
       height = 2.5,
       width = 2.5)
# 
...
boxplot <- ggpaired(a, 
  ...
  NoLegend() 
boxplot
ggsave(file.path(output.dir, "Fig2E_deg_PCAT19_adhesion.pdf"),
       plot = boxplot,
       height = 2.5,
       width = 2.5)
...
p <- VlnPlot(
  sub_seurat_obj,
  features = "GOBP-ENDOTHELIAL-CELL-PROLIFERATION",
  group.by = "MALAT1_group",
  assay = "RNA")
p
#绘制boxplot
...
boxplot <- ggpubr::ggpaired(a, 
                            ...
                            short.panel.labs = FALSE)+
  labs(title = "MALAT1",
       y = "GOBP_ENDOTHELIAL_CELL_PROLIFERATION" |>
         str_replace_all("GOBP_", "") |>
         str_replace_all("_", " ") |>
         str_to_sentence())+ 
  theme(panel.background = element_rect(fill = "white", colour = "black"),
  ...
  ggsignif::geom_signif(comparisons = list(c("high", "low")),
                        ...
                        tip_length = c(0.02, 0.02)) + 
  NoLegend() 
ggsave(file.path(output.dir, "FigS2E_deg_MALAT1_proliferation.pdf"),
       plot = boxplot,
       height = 2.5,
       width = 2.5)
...
p <- VlnPlot(
  sub_seurat_obj,
  features = "GOBP-ENDOTHELIAL-CELL-MIGRATION",
  group.by = "MALAT1_group",
  assay = "RNA")
p
#绘制boxplot
...
boxplot <- ggpubr::ggpaired(a, 
                            ...
                            short.panel.labs = FALSE)+
  labs(title = "MALAT1",
       y = "GOBP_ENDOTHELIAL_CELL_MIGRATION" |>
         str_replace_all("GOBP_", "") |>
         str_replace_all("_", " ") |>
         str_to_sentence())+ 
  theme(panel.background = element_rect(fill = "white", colour = "black"),
  ...
  ggsignif::geom_signif(comparisons = list(c("high", "low")),
                        ...
                        tip_length = c(0.02, 0.02)) + 
  NoLegend() 
ggsave(file.path(output.dir, "FigS2H_deg_MALAT1_migration.pdf"),
       plot = boxplot,
       height = 2.5,
       width = 2.5)
...
boxplot <- ggpubr::ggpaired(a, 
  ...
  ggsignif::geom_signif(comparisons = list(c("high", "low")),
                        ...
                        tip_length = c(0.02, 0.02)) + 
  NoLegend() 
ggsave(file.path(output.dir, "FigS3E_deg_MALAT1_vasculogenesis.pdf"),
       plot = boxplot,
       height = 2.5,
       width = 2.5)
...
p4
e <- p4$data
colnames(e) <- c("value","ident")
e$cell <- rownames(e)
e$gene <- "CDH13"

f <- rbind(a,b,d,e)
f$gene <- factor(f$gene, levels = c("CX3CL1","CDH13","CXCL12","HMGB1"))
p<-ggplot(f,
  ...
  xlab("MALAT1 group")+ylab("Expression")+
  theme(
...
p
ggsave(
  file.path(output.dir, "Fig2F_deg_MALAT1_CX3CL1-CDH13-CXCL12-HMGB1.pdf"),
  plot = p,
  height = 2.6,
  width = 5)
...
f <- rbind(a,b,d,e)
f$gene <- factor(f$gene, levels = c("CD74","HES1","TGFBR2","ETS1"))
p<-ggplot(f,
          aes(x=ident, y= value, fill=ident))+
  geom_violin(scale = "area", trim=F, size=0.5,color="white",cex=1,alpha=1)+ 
  geom_boxplot(width=0.1,position = position_dodge(0.9),color="white")+
  ...
  theme_classic(base_size=11, base_family = "",
                base_line_size = 0.3, base_rect_size = 0.3)+
  xlab("PCAT19 group")+ylab("Expression")+
  theme(
    legend.title = element_text(size =10, color = "black"),
    legend.text = element_text(size = 8, color = "black"),
    axis.title = element_text(size = 10,color = "black"),
...
p
ggsave(
  file.path(output.dir, "Fig2F_deg_PCAT19_CD74-ETS1-TGFBR2-HES1.pdf"),
  plot = p,
  height = 2.6,
  width = 5)
...
p
ggsave(
  file.path(output.dir, "FigS3F_deg_MALAT1_EMP2-HDAC7-QKI.pdf"),
  plot = p,
  height = 3,
  width = 4.5)
...
p1 <- PCAT19_adata |>
  cat_gseaplot(
    ...
    pvalue_table = T)
p1
ggsave(file.path(output.dir, "Fig2G_gsea_PCAT19_adhesion.pdf"),
       plot = p1,
       height = 2.2,
       width = 3.2)

MALAT1_adata <- readRDS("./results-V4-correlation/Endothelial/k_20/MALAT1_deg/C5_gsea.rds")
data <- MALAT1_adata@result
p1 <- MALAT1_adata |>
  cat_gseaplot(
    c("GOBP_ENDOTHELIAL_CELL_PROLIFERATION"),
    subplots = c(1, 2, 3),
    color = c("#66C2A5"),#颜色
    title = "GOBP_ENDOTHELIAL_CELL_PROLIFERATION" |>
      str_replace_all("GOBP_", "") |>
      str_replace_all("_", " ") |>
      str_to_sentence(),
    pvalue_table = T)
p1
ggsave(file.path(output.dir, "FigS2I_gsea_MALAT1_proliferation.pdf"),
       plot = p1,
       height = 2.2,
       width = 3.2)
...
p1 <- MALAT1_adata |>
  cat_gseaplot(
    c("GOBP_ENDOTHELIAL_CELL_MIGRATION"),
    subplots = c(1, 2, 3),
    color = c("#674AA6"),#颜色
    title = "GOBP_ENDOTHELIAL_CELL_MIGRATION" |>
      str_replace_all("GOBP_", "") |>
      str_replace_all("_", " ") |>
      str_to_sentence(),
    pvalue_table = T)
p1
ggsave(file.path(output.dir, "FigS2I_gsea_MALAT1_migration.pdf"),
       plot = p1,
       height = 2.2,
       width = 3.2)
...
library(ggplot2)
library(dplyr)
rt <- read.csv("./results-V21-Figure3/landscape_Endothelial.csv")

mycol <- c('#fddaec',"#FCCDE5",'#ffc1c1','#fbb4ae',#'#decbe4',
           '#dfc27d',"#FDB462","#FFBE7A","#f8ac8c","#82B0D2",
...
p
ggsave(file.path(output.dir, "Fig2H_flow.pdf"),
       plot = p,
       height = 3.6,
       width = 4.5)  
