...
table(metacell_obj@assays$RNA@counts['PCAT19', ] > median(metacell_obj@assays$RNA@counts['PCAT19', ]))
...
enriched <- deg_metacell_obj |>
  filter(group == "PCAT19") |>
  cat_enrich()   
saveRDS(enriched, file.path(outdir, "PCAT19_enriched.rds"))
