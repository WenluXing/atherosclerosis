...
seurat_obj_path <-
  "/home/xingwl/practice5-Atherosis/noncoding/results-V3-metacell/Endothelial/k_20/data/seurat_obj.rds"
#阴阳分组使用原来的seurat对象，高低分组使用metacell之后的对象
#但是无论使用哪一种都是这样的路径
# Ready to analyze genes?
selected_genes <- c("MALAT1")  
...
# #### Preprocessing ----
outdir <-
  file.path(outdir, selected_cell_type, str_c("k_", k), "MALAT1_deg")
...
table(metacell_obj@assays$RNA@counts['MALAT1', ] > median(metacell_obj@assays$RNA@counts['MALAT1', ]))
...
enriched <- deg_metacell_obj |>
  filter(group == "MALAT1") |>
  cat_enrich()   
saveRDS(enriched, file.path(outdir, "MALAT1_enriched.rds"))
