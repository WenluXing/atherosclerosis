...
library(tidyverse)
library(ggplot2)
output.dir <- paste0("./results-V24-Figure/Fig1") 
...
ggsave(file.path(output.dir, "Fig1A_Vein_mRNA-lncRNA.pdf"),
       plot = p,
       height = 2.5,
       width = 4.5)
...
p1
pdf(file.path(output.dir, "Fig1B_boxplot_lncRNA.pdf"),width=1,height=2.2)
print(p1)
dev.off()
...
p2
pdf(file.path(output.dir, "Fig1B_boxplot_mRNA.pdf"),width=1.15,height=2.2)
print(p2)
dev.off()
...
p1 <- ggplot(A, aes(x = factor(group, levels = c("mRNA","lncRNA")), y = num, color = group)) + 
  ...
  NoLegend()  
p1
pdf(file.path(output.dir, "FigS1A_boxplot_mRNA-lncRNA.pdf"),width=1.2,height=2.4)
print(p1)
dev.off()
...
p1 <- DimPlot(seurat.obj,
  ...
  guides(colour = guide_legend(override.aes = list(size=1), 
                               ...
                               reverse = F))  
p1
pdf(file.path(output.dir,"Fig1C_tsne_anno_mRNA.pdf"),width=2.8,height=2)
print(p1)
dev.off()
...
p2 <- DimPlot(seurat.obj,
  ...
  guides(colour = guide_legend(override.aes = list(size=1), 
                               ...
                               reverse = F))  
p2
pdf(file.path(output.dir,"Fig1C_tsne_anno_lncRNA.pdf"),width=2.8,height=2)
print(p2)
dev.off()
pdf(file.path(output.dir,"Fig1C_tsne_anno_lncRNA-参考.pdf"),width=2.8,height=2.5)
print(p2)
dev.off()
...
p1
pdf(file.path(output.dir,"Fig1D_Vlnplot_Tau-han-count.pdf"),width=1.8,height=2.2)
print(p1)
dev.off()
pdf(file.path(output.dir,"Fig1D_Vlnplot_Tau-han-count参考.pdf"),width=1.8,height=5)
print(p1)
dev.off()
...
vlo
pdf(file.path(output.dir, "Fig1E_vlnplot_lncRNA.pdf"),width=8,height=3.1)
print(vlo)
dev.off()
...
ggsave(file.path(output.dir, "Fig1F_umap_lncRNA-mRNA.pdf"),
       plot = c,
       height = 2.2,
       width = 7.2)
ggsave(file.path(output.dir, "Fig1F_umap_lncRNA-mRNA-参考.pdf"),
       plot = c,
       height = 2.2,
       width = 10)
...
p
ggsave(file.path(output.dir, "Fig1G_dotplot_lncRNA_way_cor.pdf"),
       plot = p,
       height = 5.5,
       width = 8.5)

## FigS1B ----
A <- read_csv("./results-V8-lncRNA-gene/number_T.csv")
B <- read_csv("./results-V8-lncRNA-gene/number_NK.csv")
C <- read_csv("./results-V8-lncRNA-gene/number_Pla.csv")
D <- read_csv("./results-V8-lncRNA-gene/number_B.csv")
E <- read_csv("./results-V8-lncRNA-gene/number_Mac.csv")

H <- read_csv("./results-V8-lncRNA-gene/number_Fib.csv")
I <- read_csv("./results-V8-lncRNA-gene/number_Fim.csv")
J <- read_csv("./results-V8-lncRNA-gene/number_Per.csv")
K <- read_csv("./results-V8-lncRNA-gene/number_Neu.csv")

L <- read_csv("./results-V8-lncRNA-gene/number_End.csv")
M <- read_csv("./results-V8-lncRNA-gene/number_SMC.csv")
N <- read_csv("./results-V8-lncRNA-gene/number_Mast.csv")
...
p
ggsave(file.path(output.dir, "FigS1D_dotplot_lncRNA-marker_cor.pdf"),
       plot = p,
       height = 3.5,
       width = 8.8)
...
adata_1 <- read_csv("./results-V7-hpscenic/End/tfs_targer.tsv")
adata_1 <- adata_1 %>% filter(target_gene %in% lncRNA) %>% mutate(cell_type="Endothelial")
adata_2 <- read_csv("./results-V7-hpscenic/SMC/tfs_targer.tsv")
adata_2 <- adata_2 %>% filter(target_gene %in% lncRNA)%>% mutate(cell_type="SMC")
adata_3 <- read_csv("./results-V7-hpscenic/Mac/tfs_targer.tsv")
adata_3 <- adata_3 %>% filter(target_gene %in% lncRNA)%>% mutate(cell_type="Macrophage")
adata_4 <- read_csv("./results-V7-hpscenic/Fib/tfs_targer.tsv")
...
p <- ggplot(data, aes(target_gene, tf))+
  ...
  NoLegend()
p
ggsave(file.path(output.dir,"FigS1E_tfs.pdf"),
       plot = p,
       height = 2.8,
       width = 8)
