...
seurat_obj_path <-
  "/home/xingwl/practice5-Atherosis/noncoding/results-V3-metacell/Macrophage/k_20/data/seurat_obj.rds"
#阴阳分组使用原来的seurat对象，高低分组使用metacell之后的对象
#但是无论使用哪一种都是这样的路径
# Ready to analyze genes?
selected_genes <- c("SNHG8")  
...
# #### Preprocessing ----
outdir <-
  file.path(outdir, selected_cell_type, str_c("k_", k), "SNHG8_deg")
...
table(metacell_obj@assays$RNA@counts['SNHG8', ] > median(metacell_obj@assays$RNA@counts['SNHG8', ]))
...
enriched <- deg_metacell_obj |>
  filter(group == "SNHG8") |>
  cat_enrich()   
saveRDS(enriched, file.path(outdir, "SNHG8_enriched.rds"))
