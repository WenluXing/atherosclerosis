...
P2
ggsave(file.path(output.dir, "Fig4B_san_SNHG9-proliferation.pdf"),
       plot = P2,
       height = 2,
       width = 2)
#
...
P2 <-ggExtra::ggMarginal(P, type = "histogram", 
                         ...
                         yparams = list(fill="#e23620")) 
P2
ggsave(file.path(output.dir, "Fig4B_san_SNHG9-migration.pdf"),
       plot = P2,
       height = 2,
       width = 2)
...
#
P <- expr %>%
  ggplot(aes(x = MIR100HG, y = GOBP_PLATELET_DERIVED_GROWTH_FACTOR_RECEPTOR_SIGNALING_PATHWAY)) +
  geom_point(size = 0.5,color="black",alpha = 0.9) + 
  geom_smooth(method = "lm", formula = y~x, color = "#E64B35CC", fill = "#F39B7FCC")+
  ggpubr::stat_cor(method = 'pearson',
                   aes(x = MIR100HG, y = GOBP_PLATELET_DERIVED_GROWTH_FACTOR_RECEPTOR_SIGNALING_PATHWAY),
                   label.x.npc = "left",
                   label.y.npc = "top",
                   size = 2.5,
                   color = "black") +
  ...
  theme(aspect.ratio = 1, legend.margin = margin(l=-8),
        axis.title.x = element_text(size = 8,face = "italic", color = "black"),
        axis.title.y = element_text(size = 8, color = "black")) +
  labs(y = "GOBP_PLATELET_DERIVED_GROWTH_FACTOR_\nRECEPTOR_SIGNALING_PATHWAY" |>
         str_replace_all("GOBP_", "") |>
         str_replace_all("_", " ") |>
         str_to_sentence())+
  NoLegend()
...
P2 <-ggExtra::ggMarginal(P, type = "histogram", 
                         ...
                         yparams = list(fill="#e23620")) 
P2
ggsave(file.path(output.dir, "Fig4B_san_MIR100HG-PDGFR.pdf"),
       plot = P2,
       height = 2,
       width = 2)

#
selected_genes <- c("MIR100HG","SNHG9")
seurat_obj <- readRDS("./results-V3-metacell/SMC/k_20/data/seurat_obj.rds")
...
P2 <-ggExtra::ggMarginal(P, type = "histogram",  
                         ...
                         yparams = list(fill="#e23620")) 
P2
ggsave(file.path(output.dir, "Fig4B_san_MIR100HG_HTNF.pdf"),
       plot = P2,
       height = 2,
       width = 2)
...
p
ggsave(file.path(output.dir, "Fig4C_bang_SNHG9_proliferation.pdf"),
       plot = p,
       height = 2.5,
       width = 4)
#
...
p <- a %>%
  ...
  geom_hline(yintercept = 0,lty=2,col="black",lwd=0.4)
p
ggsave(file.path(output.dir, "Fig4C_bang_SNHG9_migration.pdf"),
       plot = p,
       height = 2.5,
       width = 4)
#
gene_sets <- msigdbr::msigdbr(species = "human",
  ...
  dplyr::select(gs_name, gene_symbol)
selected_features <- gene_sets %>%
  filter(gs_name == "GOBP_PLATELET_DERIVED_GROWTH_FACTOR_RECEPTOR_SIGNALING_PATHWAY") %>%
  pull(gene_symbol)
...
p <- a %>%
  ...
  geom_segment(aes(
    ...
    size = 2) +
  theme_cat() +
  labs(title = "GOBP_PLATELET_DERIVED_GROWTH_FACTOR_RECEPTOR_SIGNALING_PATHWAY" |>
         str_replace_all("GOBP_", "") |>
         str_replace_all("_", " ") |>
         str_to_sentence(),
  ...
  geom_hline(yintercept = 0,lty=2,col="black",lwd=0.4)
p
ggsave(file.path(output.dir, "Fig4C_bang_MIR100HG_PDGFR.pdf"),
       plot = p,
       height = 2.5,
       width = 4)
#
...
p <- a %>%
  ...
  geom_hline(yintercept = 0,lty=2,col="black",lwd=0.4)
p
ggsave(file.path(output.dir, "Fig4C_bang_MIR100HG_HTNFA.pdf"),
       plot = p,
       height = 2.5,
       width = 4)
...
## FigS4A ----
selected_genes <- c("MIR100HG","SNHG9","MYC","FOXP1","CTNNB1","ADAMTS1","MEF2C",
                    "PDGFRA","PDGFRB","PDGFC","CEBPD","JUNB","EGR1",
                    "NEAT1","VTN","IGF1","PDGFD","ID1","TGIF1","WWTR1","BMPR2","TGFBR2","CCL2","KLF4")
seurat_obj <- readRDS("./results-V3-metacell/SMC/k_20/data/seurat_obj.rds")
metacell_obj <- hdWGCNA::GetMetacellObject(seurat_obj)
...
P <- expr %>%
  ggplot(aes(x = SNHG9, y = ADAMTS1)) +
  geom_point(size = 0.5,color="black",alpha = 0.9) +
  geom_smooth(method = "lm", formula = y~x, color = "#E64B35CC", fill = "#F39B7FCC")+
  ggpubr::stat_cor(method = 'pearson',
                   aes(x =  SNHG9, y = ADAMTS1),
                   label.x.npc = "left",
                   label.y.npc = "top",
                   size = 2.5,
                   color = "black") +
  ...
  NoLegend()
P
ggsave(file.path(output.dir, "FigS4A_san_SNHG9_ADAMTS1.pdf"),
       plot = P,
       height = 1.6,
       width = 1.6)
#
P <- expr %>%
  ...
  NoLegend()
P
ggsave(file.path(output.dir, "FigS4A_san_SNHG9_FOXP1.pdf"),
       plot = P,
       height = 1.6,
       width = 1.6)
...
P <- expr %>%
  ...
  NoLegend()
P
ggsave(file.path(output.dir, "FigS4A_san_SNHG9_MYC.pdf"),
       plot = P,
       height = 1.6,
       width = 1.6)
#
P <- expr %>%
  ...
  NoLegend()
P
ggsave(file.path(output.dir, "FigS4A_san_SNHG9_PDGFRB.pdf"),
       plot = P,
       height = 1.6,
       width = 1.6)
#
...
P <- expr %>%
  ggplot(aes(x = MIR100HG, y = PDGFRA)) +
  geom_point(size = 0.5,color="black",alpha = 0.9) +
  geom_smooth(method = "lm", formula = y~x, color = "#E64B35CC", fill = "#F39B7FCC")+
  ggpubr::stat_cor(method = 'pearson',
                   aes(x = MIR100HG, y = PDGFRA),
                   label.x.npc = "left",
                   label.y.npc = "top",
                   size = 2.5,
                   color = "black") +
  ...
  NoLegend()
P
ggsave(file.path(output.dir, "FigS4A_san_MIR100HG_PDGFRA.pdf"),
       plot = P,
       height = 1.6,
       width = 1.6)
#
P <- expr %>%
  ggplot(aes(x = MIR100HG, y = PDGFRB)) +
  geom_point(size = 0.5,color="black",alpha = 0.9) +
  geom_smooth(method = "lm", formula = y~x, color = "#E64B35CC", fill = "#F39B7FCC")+
  ggpubr::stat_cor(method = 'pearson',
                   aes(x = MIR100HG, y = PDGFRB),
                   label.x.npc = "left",
                   label.y.npc = "top",
                   size = 2.5,
                   color = "black") +
  ...
  NoLegend()
P
ggsave(file.path(output.dir, "FigS4A_san_MIR100HG_PDGFRB.pdf"),
       plot = P,
       height = 1.6,
       width = 1.6)
#
P <- expr %>%
  ...
  NoLegend()
P
ggsave(file.path(output.dir, "FigS4A_san_MIR100HG_CEBPD.pdf"),
       plot = P,
       height = 1.6,
       width = 1.6)
#
P <- expr %>%
  ggplot(aes(x = MIR100HG, y = JUNB)) +
  geom_point(size = 0.5,color="black",alpha = 0.9) +
  geom_smooth(method = "lm", formula = y~x, color = "#E64B35CC", fill = "#F39B7FCC")+
  ggpubr::stat_cor(method = 'pearson',
                   aes(x = MIR100HG, y = JUNB),
                   label.x.npc = "left",
                   label.y.npc = "top",
                   size = 2.5,
                   color = "black") +
  ...
  NoLegend()
P
ggsave(file.path(output.dir, "FigS4A_san_MIR100HG_JUNB.pdf"),
       plot = P,
       height = 1.6,
       width = 1.6)
...
p <- ggviolin(a, x="ident", y="value", fill = "ident", 
  ...
  labs(x = "",
       y="GOBP_SMOOTH_MUSCLE_CELL_PROLIFERATION"|>
         str_replace_all("GOBP_", "") |>
         str_replace_all("_", " ") |>
         str_to_sentence(),title="SNHG9 group")+
  NoLegend()+
  scale_y_continuous(expand = expansion(mult = c(0,0.1)))
p
ggsave(file.path(output.dir, "FigS4C_Vlnplot_SNHG9_proliferation.pdf"),
       plot = p,
       height = 2.5,
       width = 2.4)
#
...
p <- ggviolin(b, x="ident", y="value", fill = "ident", 
  ...
  theme(strip.text.x = element_text(size=9,color="black"),
        ...
        axis.title = element_text(size = 10, color = "black"))+
  labs(x = "",
       y="GOBP_SMOOTH_MUSCLE_CELL_MIGRATION"|>
         str_replace_all("GOBP_", "") |>
         str_replace_all("_", " ") |>
         str_to_sentence(),title="SNHG9 group")+
  NoLegend()+
  scale_y_continuous(expand = expansion(mult = c(0,0.1)))
p
ggsave(file.path(output.dir, "FigS4C_Vlnplot_SNHG9_migration.pdf"),
       plot = p,
       height = 2.5,
       width = 2.4)
#
...
p1 <- VlnPlot(
  sub_seurat_obj,
  features = "GOBP-PLATELET-DERIVED-GROWTH-FACTOR-RECEPTOR-SIGNALING-PATHWAY",
...
colnames(a) <- c("value","ident")
a$cell <- rownames(a)
a$way <- "GOBP_PLATELET_DERIVED_GROWTH_FACTOR_RECEPTOR_SIGNALING_PATHWAY"|>
  str_replace_all("GOBP_", "") |>
  str_replace_all("_", " ") |>
  str_to_sentence()
...
p <- ggviolin(a, x="ident", y="value", fill = "ident", 
  ...
  theme(strip.text.x = element_text(size=9,color="black"),
        ...
        axis.title = element_text(size = 10, color = "black"))+
  labs(x = "",
       y="GOBP_PLATELET_DERIVED_GROWTH_FACTOR_\nRECEPTOR_SIGNALING_PATHWAY"|>
         str_replace_all("GOBP_", "") |>
         str_replace_all("_", " ") |>
         str_to_sentence(),title="MIR100HG group")+
  NoLegend()+
  scale_y_continuous(expand = expansion(mult = c(0,0.1)))
p
ggsave(file.path(output.dir, "FigS4C_Vlnplot_MIR100HG_PDGFR.pdf"),
       plot = p,
       height = 2.5,
       width = 2.4)
#
...
p <- ggviolin(f, x="ident", y="value", fill = "ident", 
  ...
  scale_y_continuous(expand = expansion(mult = c(0,0.1)))
p
ggsave(file.path(output.dir, "FigS4C_Vlnplot_MIR100HG_HTNFA.pdf"),
       plot = p,
       height = 2.5,
       width = 2.4)
...
g$gene <- "PDGFRB"
h <- rbind(a,b,c,d,f,g)
h$gene <- factor(h$gene, levels=c("FOXP1","CTNNB1","MYC","ADAMTS1","MEF2C","PDGFRB"))
p1 <- ggplot(h,aes(x=ident,y=value,fill=ident))+
  scale_fill_manual(values = c('#eb4b3a',"#48bad0"))+
  guides(fill=guide_legend(title=x))+
  labs(x = "SNHG9 group", y = "Expression")+
  geom_boxplot(size = 0.3, width = 0.5)+ facet_wrap(~gene,nrow =1)+ theme_bw()+
  ...
  NoLegend()
p1
ggsave(file.path(output.dir, "Fig4E_boxplot_SNHG9_FOXP1-CTNNB1-MYC-ADAMTS1-MEF2C-PDGFRB.pdf"),
       plot = p1,
       height = 3,
       width = 5)
...
g$gene <- "PDGFC"
h <- rbind(a,b,c,d,f,g)
h$gene <- factor(h$gene, levels=c("PDGFRA","PDGFC","PDGFRB","CEBPD","JUNB","EGR1"))

p1 <- ggplot(h,aes(x=ident,y=value,fill=ident))+
  scale_fill_manual(values = c('#eb4b3a',"#48bad0"))+
  guides(fill=guide_legend(title=x))+
  labs(x = "MIR100HG group", y = "Expression")+
  geom_boxplot(size = 0.3, width = 0.5)+ facet_wrap(~gene,nrow =1)+ theme_bw()+
  theme(axis.line = element_blank(),
        axis.ticks = element_line(color = "black",size=0.2),
        axis.title = element_text(size = 8,color = "black"),
  ...
  NoLegend()
p1
ggsave(file.path(output.dir, "Fig4E_boxplot_MIR100HG_PDGFRA-PDGFC-PDGFRB-CEBPD-JUNB-EGR1.pdf"),
       plot = p1,
       height = 3,
       width = 5)
...
p1 <- SNHG9_adata |>
  cat_gseaplot(
    ...
    title = "GOBP_SMOOTH_MUSCLE_CELL_MIGRATION" |>
      ...
      str_to_sentence(),
    pvalue_table = T)
ggsave(file.path(output.dir, "FigS4D_gsea_SNHG9_SMC_Migration.pdf"),
       plot = p1,
       height = 2.4,
       width = 3)
#
p2 <- SNHG9_adata |>
  cat_gseaplot(
    ...
    title = "GOBP_SMOOTH_MUSCLE_CELL_PROLIFERATION" |>
      ...
      str_to_sentence(),
    pvalue_table = T)
ggsave( file.path(output.dir, "FigS4D_gsea_SNHG9_SMC_Proliferation.pdf"),
        plot = p2,
        height = 2.4,
        width = 3)
#
MIR100HG_adata <- readRDS("./results-V4-correlation/SMC/k_20/MIR100HG_deg/C5_gsea.rds")
p4 <- MIR100HG_adata |>
  cat_gseaplot(
    c("GOBP_PLATELET_DERIVED_GROWTH_FACTOR_RECEPTOR_SIGNALING_PATHWAY"),
    subplots = c(1, 2, 3),
    color = c("#FF6600"),#颜色
    title = "GOBP_PLATELET_DERIVED_GROWTH_FACTOR_RECEPTOR_SIGNALING_PATHWAY" |>
      str_replace_all("GOBP_", "") |>
      str_replace_all("_", " ") |>
      str_to_sentence(),
    pvalue_table = T)
p4
ggsave( file.path(output.dir, "FigS4D_gsea_MIR100HG_PDGFR.pdf"),
        plot = p4,
        height = 2.4,
        width = 3)
#
...
p3 <- MIR100HG_adata |>
  cat_gseaplot(
    ...
    pvalue_table = T)
p3
ggsave(file.path(output.dir, "FigS4D_gsea_MIR100HG_TNFA.pdf"),
       plot = p3,
       height = 2.4,
       width = 3)
...
mycol <- c("#FFB9B9","#FCCDE5","#FDE0DF","#FDB462","#FFBE7A", "#f8ac8c",
           "#B3DE69","#FCCDE5","#CCEBC5","#E6F5D0","#FCCDE5","#BEB8DC","#80B1D3",
           "#8ECFC9","#8ECFC9","#8ECFC9","#FCCDE5") 
...
p
ggsave(file.path(output.dir, "FigS4G_flow.pdf"),
       plot = p,
       height = 4,
       width = 5.5) 

## Fig4F Fig4G FigS4E FigS4F ----
library(WGCNA)
library(dplyr)
output.dir <- paste0("./results-V23-Figure/Fig5/wgcna_pre2") 
...
text(sft$fitIndices[, 1],-sign(sft$fitIndices[, 3]) * sft$fitIndices[, 2],
     ...
     cex = cex1,
     col = "red") +
  abline(h = 0.8, col = "red")# this line corresponds to using an R^2 cut-off of
...
MEList <- moduleEigengenes(expr, colors = dynamicColors) #特别费时间
# saveRDS(MEList,file.path(output.dir,"MEList.rds"))

MEs <- MEList$eigengenes
MEs$MEgrey <- "0" #将NA值换为0
...
P3 <- labeledHeatmap(
  ...
  #WGCNA提醒greenWhiteRed不适合红绿色盲，建议用blueWhiteRed
...
# 模块内分析：作模块membership和genesignificance的相关图
module = "yellow"
