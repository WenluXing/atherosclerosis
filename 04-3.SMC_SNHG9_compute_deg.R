...
# #### Preprocessing ----
outdir <-
  file.path(outdir, selected_cell_type, str_c("k_", k), "SNHG9_deg")
...
table(metacell_obj@assays$RNA@counts['SNHG9', ] > median(metacell_obj@assays$RNA@counts['SNHG9', ]))
...
enriched <- deg_metacell_obj |>
  filter(group == "SNHG9") |>
  cat_enrich()   
saveRDS(enriched, file.path(outdir, "SNHG9_enriched.rds"))
