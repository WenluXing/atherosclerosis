...
# #### Preprocessing ----
outdir <-
  file.path(outdir, selected_cell_type, str_c("k_", k), "MIR100HG_deg")
...
table(metacell_obj@assays$RNA@counts['MIR100HG', ] > median(metacell_obj@assays$RNA@counts['MIR100HG', ]))
write_csv(deg_metacell_obj, file.path(outdir, "deg_metacell_obj.csv"))

#### Function enrich ----
enriched <- deg_metacell_obj |>
  filter(group == "MIR100HG") |>
  cat_enrich()   
saveRDS(enriched, file.path(outdir, "MIR100HG_enriched.rds"))
